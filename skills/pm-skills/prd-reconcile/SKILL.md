---
name: prd-reconcile
description: "PRD合并与消歧 — 将多份分批构建的PRD/需求文档合并为一份全局PRD。系统化识别跨文档冲突，引导逐一决策，产出无歧义的统一文档。Make sure to use this skill whenever the user has multiple PRD or requirement documents that need merging, mentions cross-document conflicts or inconsistencies, says they need a 'global PRD' or 'unified PRD', or has incrementally-built documents that need reconciliation — even if they just say 'these documents conflict' or 'merge these docs' without explicitly asking for a 'PRD reconcile'."
---

# PRD Reconcile — 多文档合并与消歧

将分批构建的多份PRD/需求文档合并为一份权威的全局PRD。系统化识别冲突、引导逐一决策、确保合并后无歧义。

## 适用场景

- 多份PRD（主文档、页面PRD、模块PRD、字段定义CSV、校验规则Excel等）分批构建，需要合并
- 子文档之间存在矛盾、术语冲突、流程不一致
- 需要产出所有团队可对齐的权威全局PRD

## 前置

- **pm-knowledge 已完成摄入** — 所有源文档已通过 pm-knowledge ingest 写入 `.pm-wiki/`（由 pm-workflow 阶段 0 统一执行）
- 项目知识库 `<project>/.pm-wiki/` 已存在且有结构化内容

---

## 五阶段流程

### Phase 1: 冲突分析

全量交叉比对，识别冲突点并分级编号。

**冲突类型：**
| 类型 | 说明 | 示例 |
|------|------|------|
| 矛盾 | 同一概念不同定义 | 状态枚举不同、角色名称不同 |
| 不一致 | 同一流程步骤/字段不同 | 流程顺序差异、字段列表差异 |
| 术语冲突 | 同一实体不同称呼 | "A岗" vs "预检员" vs "role1" |
| 缺失 | 某文档提到其他未覆盖 | 某字段只在一份文档出现 |
| 隐含歧义 | 看似一致实则不同理解 | "挂起"是否回退主任务状态 |

**分级标准：**
| 级别 | 含义 | 合并影响 |
|------|------|---------|
| P0 | 核心架构冲突 | 不决策就无法合并 |
| P1 | 重要业务逻辑差异 | 影响开发实现 |
| P2 | 术语/命名/展示差异 | 影响沟通但非功能 |
| P3 | 细微差异/风格偏好 | 合并时统一即可 |

**编号规则**：C1, C2, C3... 连续编号。多轮排查时编号续接（第二轮从上一轮最大编号+1开始）。

**排查流程：**
1. 第一轮：逐段落交叉比对所有文档，识别明显矛盾 → 编号分级 → 写入报告
2. 第二轮：回源原始文档逐段落重新提取，检查隐含冲突 → 续接编号
3. 如有需要可继续第三轮，直到用户确认无遗漏

**报告格式**：

> 冲突分析报告由 pm-knowledge ingest 管理存储路径和 frontmatter 格式。以下仅定义报告内容结构。

| 编号 | 冲突描述 | 涉及文档 | 建议方向 |
|------|---------|---------|---------|

**P0 级冲突（必须决策）**

**P1 级冲突（重要差异）**

**P2-P3 级冲突（合并时统一）**

产出：冲突分析报告，编号+分级+涉及文档+建议方向。

### Phase 2: 逐一决策

与用户逐一讨论每个冲突，记录决策和理由。

**呈现格式** — 每个冲突向用户展示：
- 编号和描述
- 涉及文档的原文对比（不是摘要，是原文引用）
- ≥2个决策选项 + 利弊分析
- 推荐方向（如有）+推荐理由

**讨论顺序**：P0 → P1 → P2-P3。P0决策完成后检查是否影响其他冲突的建议方向。

**多轮衔接**：Phase 1和Phase 2交替——发现冲突→编号→讨论→决策→再排查→新冲突→继续，直到用户确认"没有更多冲突了"。

**复杂系统建模**：如果冲突涉及状态转换、权限矩阵等系统性问题，单独产出完整矩阵写入决策记录。

**决策记录格式**：

> 决策记录由 pm-knowledge ingest 管理存储路径和 frontmatter 格式。以下仅定义记录内容结构。

| 编号 | 冲突 | 决策 |
|------|------|------|

**P0 级决策（已确认）**

**P1 级决策（已确认）**

（每个决策附 **Why:** 理由 和 **How to apply:** 适用场景）

产出：决策记录，全部confirmed。

### Phase 3: 合并编写

基于所有源文档和决策记录编写全局PRD。

1. **设计章节结构**：根据源文档覆盖范围和决策结果，设计合并后的大纲。不要照搬任何单份文档的结构，而是基于所有文档的综合覆盖面重新设计
2. **逐章编写**：
   - 冲突内容取决策结果，不取任何单文档的说法
   - 标注决策编号引用方便回溯
   - 统一术语（按术语决策统一用词）
   - 状态枚举、字段命名统一
3. **系统性内容完整性**：确保状态转换矩阵、角色权限、数据模型等系统性内容完整呈现，不留半成品
4. **写入** `<项目名> - 全局PRD.md` 到项目根目录

产出：全局PRD文档。

### Phase 4: 信息完整度比对与补全

合并编写后，系统化比对每份源文档的细节信息在全局PRD中的保留程度，自动补全缺失细节。

**问题**：合并时聚焦冲突解决和高层次整合，容易丢失源文档中的细节设计——字段校验规则、交互状态、边界条件、错误提示、条件逻辑等。

#### 4.1 信息清单提取

从每份源文档提取原子信息项，构建信息清单：

| 维度 | 信息类型 | 示例 |
|------|---------|------|
| 字段定义 | 名称、类型、长度、格式、默认值、允许值 | 用户名: 2-20字符, 不能含特殊符号 |
| 校验规则 | 提交校验、实时校验、条件校验、错误提示 | 手机号提交时格式校验, 错误提示"请输入11位数字" |
| 状态转换 | 触发条件、前置条件、权限要求、副作用 | 审核通过→已发布, 需管理员权限, 通知申请人 |
| 交互细节 | 加载态、禁用条件、悬停态、空态、异常态 | 列表为空时显示"暂无数据", 加载中显示骨架屏 |
| 条件逻辑 | 分支条件、组合条件、互斥条件 | 当A岗通过且B岗未拒绝时, 进入"待B岗审核" |
| 边界情况 | 极值、空值、并发、超时、回退 | 同一用户并发提交时提示"请勿重复操作" |
| 非功能约束 | 性能要求、安全要求、兼容性 | 页面加载<3s, IE11兼容, 密码bcrypt加密 |
| 数据关系 | 外键、级联、依赖、引用 | 删除节点时级联挂起其下所有子任务 |

#### 4.2 逐项比对

对每条信息项，检查在全局PRD中的保留程度：

| 完整度 | 定义 | 处理方式 |
|--------|------|---------|
| 完整 | 全局PRD包含该信息的所有细节 | 无需处理 |
| 部分 | 全局PRD提到了该信息但缺少细节 | 自动补全缺失细节 |
| 缺失 | 全局PRD完全没有该信息 | 自动补全（如无冲突）或标记待审 |
| 冲突 | 补全会与全局PRD其他内容矛盾 | 标记待审，引导决策 |

**比对方法**：
1. 逐份源文档 → 逐信息维度 → 逐信息项 → 在全局PRD中搜索对应描述
2. 使用 pm-knowledge query 检索全局PRD中是否覆盖该信息
3. 如找到对应描述，对比细节是否完整
4. 如未找到，检查是否被其他源文档的同维度信息覆盖

#### 4.3 自动补全

对"部分"和"缺失"（且无冲突）的信息项，直接补入全局PRD：

**补全策略**：
- 缺失且无冲突 → 在全局PRD对应章节中插入完整描述
- 部分且无冲突 → 在全局PRD现有描述后追加缺失的细节
- 补全时标注来源：`(来源: <源文档名> §<章节>)`

**插入位置规则**：

| 信息维度 | 插入位置 |
|---------|---------|
| 字段定义 | 对应功能需求的字段表格 |
| 校验规则 | 字段表格的校验列或独立校验规则章节 |
| 状态转换 | 状态转换矩阵或流程描述 |
| 交互细节 | UI规范或交互说明 |
| 条件逻辑 | 业务逻辑描述 |
| 边界情况 | 对应功能的边界情况小节 |
| 非功能约束 | 非功能需求章节 |
| 数据关系 | 数据模型定义 |

#### 4.4 完整度报告

> 完整度报告由 pm-knowledge ingest 管理存储路径和 frontmatter 格式。以下仅定义报告内容结构。

**统计概览**
| 源文档 | 信息项总数 | 完整 | 部分 | 缺失 | 冲突 |
|--------|----------|------|------|------|------|

**已自动补全**
| 信息项 | 来源 | 补入位置 | 补全内容摘要 |
|--------|------|---------|-------------|

**待人工审核**
| 信息项 | 来源 | 冲突描述 | 建议方向 |
|--------|------|---------|---------|

#### 4.5 人工审核

仅"冲突"类项需用户确认：
- 展示冲突细节 + 全局PRD中的矛盾点
- 提供合并方案选项
- 用户决策后补充到全局PRD

自动补全的内容默认写入，用户可在最终验证（Phase 5）时审查。

### Phase 5: 验证

对照决策记录和完整度报告逐条验证全局PRD。

**验证清单：**
1. 决策覆盖 — 逐C编号检查是否体现
2. 术语一致性 — 搜索废弃旧术语应全部替换
3. 系统性内容完整性 — 状态矩阵/权限矩阵/数据模型无遗漏
4. 冲突残留 — 全文自查无未解决矛盾
5. 源文档覆盖 — 每份源文档核心内容都有对应章节
6. 细节完整度 — 完整度报告中"部分"/"缺失"项均已补全
7. 补全准确性 — 自动补全的内容与源文档原文一致，无曲解

遗漏→回Phase 3/4补充→重新验证。全部通过→最终版确认。

---

## 关键原则

- **不遗漏**：全量逐段落交叉比对，不接受"差不多"判断
- **不丢细节**：合并不是抽象——字段校验、交互状态、边界条件等细节设计必须完整保留，通过完整度比对自动补全
- **不猜测**：每个冲突必须与用户讨论确认，不自作主张
- **不遗忘**：决策编号+理由+PRD引用，全程可回溯
- **不歧义**：合并后术语统一、枚举统一、流程唯一
- **多轮迭代**：冲突排查不是一次性的——分析→决策→再排查，直到用户确认无遗漏