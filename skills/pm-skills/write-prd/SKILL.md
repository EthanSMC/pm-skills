...
4. **撰写 PRD** — 按上述结构生成文档
4.5. **FR 完整度验证** — 对照 `.pm-wiki/requirements/` 中的源知识页面，逐条检查：
   - 每个已知字段定义是否在 FR 表中有对应需求
   - 校验规则、边界条件是否在验收标准中体现
   - 状态转换是否在功能需求或非功能需求中覆盖
   - 知识注入标记的"高细节区域"是否全部体现在 PRD 中
   - 发现遗漏 → 补入 FR 表或对应章节，标注 `(来源: <wiki页面>)`
...
- **完整度回源** — PRD 写完后必须对照 wiki 源页面验证字段/规则/边界条件的覆盖度，不遗漏已知信息
