---
name: prototyping
description: "原型构建（内部子 skill，由 pm-workflow 调度）— 基于 PRD 创建技术规格、实施计划和骨架代码。验证架构可行性，不追求完整实现。"
---
...
pm-workflow 在阶段 3（原型验证）使用 Skill 工具调用此 skill。
...
- 原型产出文件清单（spec.md、plan.md、scaffold-index.md 等）
- 架构验证结论（哪些场景可走通、哪些需要调整）
- 验证结果摘要

### 子阶段门控

prototyping 内部管理子阶段门控（技术规格→前端设计→实施计划→骨架构建→审查→验证→分支管理），pm-workflow 不干预子阶段进度。
