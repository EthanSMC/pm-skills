...
写入 wiki 页面后，回源到原始文档检查信息捕获完整度。

**比对维度**（与 prd-reconcile Phase 4 一致）：

| 维度 | 信息类型 | 示例 |
|------|---------|------|
| 字段定义 | 名称、类型、长度、格式、默认值、允许值 | 用户名: 2-20字符 |
| 校验规则 | 提交校验、实时校验、条件校验、错误提示 | 手机号提交时格式校验 |
| 状态转换 | 触发条件、前置条件、权限要求、副作用 | 审核通过→已发布 |
| 交互细节 | 加载态、禁用条件、空态、异常态 | 列表为空显示"暂无数据" |
| 条件逻辑 | 分支条件、组合条件、互斥条件 | 当A岗通过且B岗未拒绝时 |
| 边界情况 | 极值、空值、并发、超时 | 并发提交提示"请勿重复操作" |
| 非功能约束 | 性能、安全、兼容性 | 页面加载<3s |
| 数据关系 | 外键、级联、依赖 | 删除节点时级联挂起子任务 |

**比对流程**：
1. 对刚写入的 wiki 页面，逐维度提取信息项
2. 回源到原始文档（doc_read），搜索对应描述
3. 标记每条信息项的完整度：
   - 完整 → 无需处理
   - 部分 → wiki 提到了但缺细节 → 自动补入缺失细节
   - 缺失 → wiki 完全没有 → 自动补入（标注来源）
   - 冲突 → 补全会与 wiki 其他内容矛盾 → 标记待审

**补全规则**：
- 补入时标注来源：`(来源: <源文件名> §<章节/页>)`
- 冲突类项不自动补入，写入 `_working/` 待审列表，等待用户决策
- 补入后重新检查该页面的 frontmatter 状态（如从 confirmed 变为 pending-review）
...
输出格式（注入到 pm-brainstorming 上下文）— 双层结构（摘要 + 高细节区域）：
```
## 已有知识基础
...
### ⚠ 高细节区域（建议深读源文档）
以下内容细节密度高，摘要无法完整覆盖，请在设计时回源确认：

| 主题 | 涉及 wiki 页面 | 细节维度 | 源文档 |
|------|--------------|---------|--------|
| [字段定义类] | requirements/字段规格.md | 字段定义+校验规则 | 源PRD §3.2 |
| [状态转换类] | requirements/流程定义.md | 状态转换+条件逻辑 | PRD §4.1 |
...
- **完整度校验不只在 reconcile** — ingest 和每个下游 skill 都应做回源完整性比对，不让压缩后的知识成为唯一输入
