...
PM Skills 的核心内容是 Markdown 指令文件（SKILL.md），任何 AI coding agent 都能读取和遵循。不同 agent 的安装方式略有不同，选你用的那个：
...
Gemini CLI 支持 `.gemini/skills/` 目录，每个 skill 是一个 `.md` 文件：

```bash
# 1. 下载仓库
git clone https://github.com/EthanSMC/pm-skills.git

# 2. 把每个 SKILL.md 复制到项目的 .gemini/skills/ 下（去掉 frontmatter 中的 YAML，只保留 Markdown 正文）
#    frontmatter 是 Claude Code 专有格式，Gemini 不需要
...
> **注意：** 每个文件开头的 YAML frontmatter（`---` 包裹的 `name:` 和 `description:` 行）是 Claude Code 专有格式。Gemini CLI 不识别 frontmatter，复制后需手动删除每个文件开头的 3 行（从 `---` 到 `---`）。

同时把 `pm-knowledge/scripts/` 和 `visual-companion/scripts/` 也复制到项目中（路径不变）。

### OpenCode

OpenCode 用 `.opencode/commands/` 目录，子目录创建嵌套命令 ID：

```bash
# 1. 下载仓库
git clone https://github.com/EthanSMC/pm-skills.git

# 2. 把 SKILL.md 复制到 .opencode/commands/pm/ 下（去掉 frontmatter）
cp pm-skills/skills/pm-skills/pm-workflow/SKILL.md      你的项目/.opencode/commands/pm/workflow.md
cp pm-skills/skills/pm-skills/pm-knowledge/SKILL.md     你的项目/.opencode/commands/pm/knowledge.md
cp pm-skills/skills/pm-skills/pm-personalize/SKILL.md   你的项目/.opencode/commands/pm/personalize.md
cp pm-skills/skills/pm-skills/prd-reconcile/SKILL.md    你的项目/.opencode/commands/pm/reconcile.md
cp pm-skills/skills/pm-skills/pm-brainstorming/SKILL.md 你的项目/.opencode/commands/pm/brainstorming.md
cp pm-skills/skills/pm-skills/visual-companion/SKILL.md 你的项目/.opencode/commands/pm/visual-companion.md
cp pm-skills/skills/pm-skills/write-prd/SKILL.md        你的项目/.opencode/commands/pm/write-prd.md
cp pm-skills/skills/pm-skills/prototyping/SKILL.md      你的项目/.opencode/commands/pm/prototyping.md
```

> 同样需要去掉每个文件开头的 YAML frontmatter（`---` 到 `---` 共 3 行）。

安装后调用方式为 `project:pm:workflow`、`project:pm:brainstorming` 等。

### Codex CLI（OpenAI）

Codex CLI 没有 skills 目录机制，PM Skills 的指令需要写入项目级配置文件：

```bash
# 1. 下载仓库
git clone https://github.com/EthanSMC/pm-skills.git

# 2. 把所有 SKILL.md 内容合并到项目的 AGENTS.md
#    删掉 frontmatter，以 ## 标题分隔每个 skill
cat pm-skills/skills/pm-skills/pm-workflow/SKILL.md \
    pm-skills/skills/pm-skills/pm-knowledge/SKILL.md \
    pm-skills/skills/pm-skills/pm-brainstorming/SKILL.md \
    pm-skills/skills/pm-skills/write-prd/SKILL.md \
    pm-skills/skills/pm-skills/prd-reconcile/SKILL.md \
    pm-skills/skills/pm-skills/pm-personalize/SKILL.md \
    pm-skills/skills/pm-skills/prototyping/SKILL.md \
    pm-skills/skills/pm-skills/visual-companion/SKILL.md \
    | sed '/^---$/,/^---$/d' > 你的项目/.codex/instructions.md
```

> 也可以把内容直接写在项目根目录的 `AGENTS.md` 中。

### Kimi Code

Kimi Code 用 `kimi.md` 作为项目指令文件，同样需要合并：

```bash
# 1. 下载仓库
git clone https://github.com/EthanSMC/pm-skills.git

# 2. 把所有 SKILL.md 内容合并到项目的 kimi.md
#    删掉 frontmatter，以 ## 标题分隔每个 skill
cat pm-skills/skills/pm-skills/pm-workflow/SKILL.md \
    pm-skills/skills/pm-skills/pm-knowledge/SKILL.md \
    pm-skills/skills/pm-skills/pm-brainstorming/SKILL.md \
...
PM Skills 的 SKILL.md 是纯 Markdown 指令文件。任何能读取项目级指令文件的 agent 都能用：

- **有 skills/commands 目录的**：把 SKILL.md 复制到对应目录，去掉 YAML frontmatter
- **用单文件指令的**（如 `AGENTS.md`、`kimi.md`）：把所有 SKILL.md 内容合并到指令文件，去掉 frontmatter

通用原则：**YAML frontmatter（`---` 包裹的 3 行）是 Claude Code 专有格式，其他 agent 需要删除。Markdown 正文是通用的，任何 agent 都能理解。**
...
**Skill/命令不生效** → 检查指令文件是否在正确的目录下、frontmatter 是否已删除（非 Claude Code 用户）
...
**YAML frontmatter 是什么？** → 每个 SKILL.md 开头有 3 行 `---` 包裹的内容（name 和 description），这是 Claude Code 专有格式。其他 agent 不识别，安装时需删除这 3 行。
